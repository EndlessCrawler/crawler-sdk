...
- **No raw generic read route.** The keep-lights-on `/api/read/<chainId>/<contractName>/<fn>/<args…>` dynamic-dispatch route died with the explorer rebuild (P8) — raw contract reads are what `crawler-api` itself is for.
