...
1. **Cache — one private, contract-agnostic package** (`cache`) archiving `tokenURI` output for every SDK world contract. Per token, exactly **two committed files**, plus a per-network **provenance/state file**, under one directory per world:

   ```
   cache/worlds.json                              # registry: world name → { dataDir, rpcEnv }
   cache/data/<dataDir>/<network>/<tokenId>.json  # the tokenURI metadata JSON, formatted for readability
   cache/data/<dataDir>/<network>/<tokenId>.svg   # the decoded original SVG, formatted
   cache/data/<dataDir>/<network>/_cache.json     # fetch provenance + state (below)
   ```

   - **`worlds.json` — the lean registry.** Keyed by world `name` → `{ dataDir, rpcEnv }` (`rpcEnv` is the RPC **env-var name**, never a secret). Its keyset **is** the cache coverage — goerli is a `crawler-data` world but is never listed, so it is never cached. Everything else (network, chainId, contract address, ABI) comes from the `crawler-data` world resolved by that name + `crawler-api`'s `getWorldContract(world)`; the binding has **one home** (the world) and is never restated in the registry.
   - The `.json` is the tokenURI JSON **with its data-URI blob fields extracted**: `image` is decoded into the sibling `.svg`; `animation_url` is dropped (same SVG in an HTML player — fully derivable). Everything else is stored as returned. Files are byte-stable via the canonical serializer.
   - **`_cache.json` — provenance + fetch state**, one per network directory, byte-stable via the canonical serializer, excluded from the token-contiguity invariant (leading `_`). It echoes the source binding (world `name`, `network`, `chainId`, `contractName`, `contractAddress`) so the archive is self-describing about where it came from, plus `fetchedThroughBlock`, `updatedAt`, and a `tokens` map of `tokenId → { block, fetchedAt }` (decimal-string block, ISO-8601 UTC time).
   - **Fetch strategy — missing-only, block-pinned, idempotent.** No struct calls, no manifest of the fetch *list* — file presence is the record. Each run **pins one block `B`** at the start and reads every `tokenURI` `at` `B` (a single consistent snapshot); fetch list = on-chain `1..totalSupply` **minus** the token files already on disk; each fetched token is stamped `{ block: B, fetchedAt }`. `fetchedThroughBlock` advances to `B` on **every** clean run (even when nothing was fetched), so a future staleness scan starts from `B+1`. A second run fetches nothing (only the watermark moves).
   ...
   - RPC is caller-supplied (the `worlds.json` env var) with the api's warned public-RPC fallback; **one generic script** fetches over every registered world.
