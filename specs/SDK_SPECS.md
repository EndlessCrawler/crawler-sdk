...
|---|---|---|
| `packages/crawler-utils` | **`@avante/crawler-utils`** | The SDK's generic, non-domain primitives, **multi-entry** (one subpath per module, mirroring `crawler-data`) so a consumer bundles only what it imports: **`/bi`** (the `BigIntish`/`HexString` types + the bigint-handling functions — the base of the layering, imported by everyone), **`/format`** (`formatJSON` the canonical JSON printer, `serialize`/`deserialize` the bigint-round-tripping codec, `formatXML` the zero-dep re-indenter), **`/encode`** (base64 + data-URI codecs), **`/seeder`** (`makeSeed` — keccak256 over a canonical string). **Zero runtime dependencies** (`js-sha3` is a devDependency tsdown bundles into the `/seeder` chunk); `sideEffects: false`; Node-compatible. Nothing here imports another `@avante/*` package — it is the bottom of the graph (`utils ← core ← api/data/react`). |
| `packages/crawler-core` | **`@avante/crawler-core`** | The heart: the **`bi` namespace** (re-exported from `@avante/crawler-utils/bi` — a core subpath shim, so `import { bi } from '@avante/crawler-core'` is unchanged); schemas (`ec`, `cnc`); `World`/`View` types + read functions; coordinate math (Compass ↔ Coord ↔ Slug, neighbor offsets); the `Crawler` client (handle + container — see §The `Crawler` client); chamber locators (`resolveCoord`) and the schema invalidation primitive (`getInvalidatedCoords`). **No third-party runtime deps; peers `@avante/crawler-utils` (itself zero-dep); Node-compatible (no browser APIs).** Single root export (the legacy `./internal` importer subpath died with the global store). |
| `packages/crawler-data` | **`@avante/crawler-data`** | Static worlds as **one subpath export per world** (`@avante/crawler-data/mainnet`, `/goerli`, `/sepolia`; a `cnc` world in v1) — the root exports **no world JSON**, so bundles carry exactly the worlds imported; each world export **bundles its schema's converter** alongside the world data (see §The `Crawler` client); per-schema **converters** (pure: token payload → `ChamberData<Schema>`; payload types live beside them); the **builder** (build-time module: reads `cache/data`, converts, writes world JSON via the canonical serializer — `formatViewData`, imported from `crawler-api`). **Zero runtime deps** — `crawler-api` is a devDependency used by the build script only (it also supplies `formatViewData`; no formatter dependency enters `crawler-data`). |
| `packages/crawler-api` | **`@avante/crawler-api`** | On-chain layer, viem 2 only — a **pure contract interface** (see §`crawler-api`): fully-typed per-world viem contract instances + parsed-result helpers (`tokenURI` / `ownerOf` / `totalSupply`; caller-supplied viem client or RPC url, or a warned public fallback — never ours); known & generic non-chamber contract helpers (`getCardsContract()`, ERC-20/ERC-721); the **live watcher** (poll-based mint detection) + the **per-schema payload assemblers** — the SDK's single fetch/assembly implementation, consumed by the cache fetch and the live path alike; **owner helpers** scoped to the connected player + delegate.xyz (#17); the **canonical serializer `formatViewData`** (see §Canonical serialization — the package's one non-contract member; must not be removed from this package; a thin sync wrapper over `@avante/crawler-utils`'s `formatJSON`). Depends on core + `@avante/crawler-utils` (+ viem); **no `prettier`**. |
...
flowchart TD
  subgraph pub ["published @avante/* — runtime deps"]
    utils["crawler-utils<br/>(zero deps, multi-entry)"]
    core["crawler-core<br/>(Node-compatible)"]
    data["crawler-data<br/>(zero runtime deps)"]
    api["crawler-api<br/>(+ viem 2)"]
...
Rules: root entry points only, plus the two multi-entry packages' subpaths — `crawler-data`'s per-world subpaths and `crawler-utils`'s per-module subpaths (`/bi`, `/format`, `/encode`, `/seeder`) — no other subpath exports in v1 (the per-view world split stays a data-layout escape hatch, §Views). Every listed export carries complete TSDoc (§Type-system rules); an export not listed here is not public.

### `@avante/crawler-utils` (multi-entry — one subpath per module)

- **`/bi`** — functions `toBigInt`, `toHex`, `toAddress`, `toByteArray`, `toNumberArray`, `toDecimalString`, `equals`, `fromBinaryArray`, guards `isBigInt`, `isBigIntish`, `isHexString`; error `InvalidBigIntishError`; types `BigIntish`, `HexString`. (`crawler-core` re-exports these as the `bi` namespace + top-level types — §`@avante/crawler-core`.)
- **`/format`** — `formatJSON` (§Canonical serialization), `serialize`, `deserialize`, `formatXML`; type `FormatXmlOptions`.
- **`/encode`** — `encodeBase64`, `decodeBase64`, `decodeBase64Text`, `encodeDataUri`, `decodeDataUri`; type `EncodeDataUriOptions`.
- **`/seeder`** — `makeSeed`.
- The convenience root barrel (`@avante/crawler-utils`) re-exports every module but is **not** a documented reference page — the four subpaths are the documented surface.
...
- **Home:** a dedicated, self-contained module — `@avante/crawler-utils/bi` (`packages/crawler-utils/src/bi/`) — types, conversions, guards, and its own exhaustive unit tests, at the bottom of the dependency graph (zero deps). `crawler-core` **re-exports it** via a subpath shim (`src/bigintish/`), so **the functions ship as the `bi` namespace export** (`import { bi } from '@avante/crawler-core'`, unchanged; member list in §Public surface) and the types + `InvalidBigIntishError` stay top-level named exports of core. Consumers may also import the raw functions from `@avante/crawler-utils/bi` directly. No other module reimplements bigint handling.
- **Spelling:** `BigIntish`.
...
1. **Cache — one private, contract-agnostic package** (`cache`) archiving `tokenURI` output for every SDK world contract. Per token, **two committed files** — three for `ec`-schema worlds — plus a per-`dataDir` **provenance/state file**, under one directory per world:
   ...
   cache/data/<dataDir>/<tokenId>.svg   # the decoded SVG, re-indented (formatXML)
   cache/data/<dataDir>/<tokenId>.html  # ec only: the decoded animation_url player, re-indented
   ...
   - The `.svg` is the decoded `image` **re-indented with `@avante/crawler-utils`'s `formatXML`** (zero-dep tokenizing indenter) for a readable, diff-friendly, byte-stable archive. It is therefore *reflowed*, not byte-identical to the on-chain original (it renders identically); this is the form the builder carries into the world's `TokenSvg` view and that consumers serve.
   - The `.html` (`ec` worlds) is the decoded `animation_url` — **the chain's own playable form** (the same SVG in an HTML player) — re-indented like the `.svg`. Archived for reference; it never ships in a world (see §Token SVGs).
...
- **`formatViewData` in `crawler-api`** (`src/lib/utils/formatter.ts`, exported from the package root) is the canonical dataset serializer and **must not be removed from that package**. It is a thin **synchronous** wrapper (`(data?) => string`) over `@avante/crawler-utils`'s `formatJSON`, keeping only the historical scalar/empty short-circuits (a bare string passes through; a non-object stringifies).
- **Home rationale (the api's one non-contract member):** serialization only ever happens beside on-chain fetching — the builder and cache tooling both sit where `crawler-api` is already present. Nothing serializes at runtime (`world.import` merges in memory).
- **`formatJSON` (in `@avante/crawler-utils/format`)** is the layout engine — a zero-dependency printer **byte-for-byte identical** to the former prettier-JSON output (tab indent, `printWidth` 80, short arrays/objects inline, all-numeric arrays packed many-per-line via a prettier-faithful `fill`, single trailing newline). Byte-identity is the standing guard: rebuilding `mainnet.json` through the builder must diff empty (modulo the re-stamped `timestamp`).
...
- **No `prettier` anywhere in the SDK's runtime or tooling** — `formatJSON` replaced it in `crawler-api`, and `formatXML` (also `@avante/crawler-utils/format`) replaced it in the `cache` SVG/HTML archive. `crawler-utils` carries zero runtime deps, so nothing enters consumer bundles; `bigint` round-tripping for machine-facing state (react persistence) uses `serialize`/`deserialize` from the same module.
- No `BigInt.prototype.toJSON` monkeypatch — `bigint` handling is local to the formatter (every package declares `sideEffects: false`).
