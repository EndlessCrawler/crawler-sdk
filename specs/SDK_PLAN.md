...
**Status:** _Implementation underway — **P1–P8 landed**_ (core rewritten to the settled spec, worlds migrated, the api contract layer + live surface, the EC on-chain cache with its staleness pass, the EC converter, the builder + per-world exports, the full react surface + live path, and the explorer rebuilt on the new bindings; execution detail in `specs/SDK_REFACTOR.md`). This refactor runs **after** the V2 modernization (`specs/V2_PLAN.md`) landed its stack work, but **before anything is published to npm** — the first publish happens only once this refactor is done (P9). V2 made the stack modern and correct; this plan makes the *API* the one we actually want, and that shipped API is what goes to npm.
...
**Settled → SPECS §Data pipeline & chamber sources:** provenance (every chamber comes from an ERC-721 token contract; the World binding); the pipeline — a single private `cache` package (pure `tokenURI` archive + missing-only fetch + the schema-policy staleness pass — #21/#16) → per-schema pure converters in `crawler-data` (struct-fed token payload in, single-place-to-fetch / single-place-to-convert rule) → the builder assembling world JSON via the canonical serializer (mainnet only — goerli frozen as migrated, sepolia when a deployment exists — #22); the live path (poll-based watcher + per-schema payload assemblers in `crawler-api`, localStorage persistence in `crawler-react` — #16); the publish cadence; and the three tiers materializing via `world.import` + pure merge (no read-through source interface).

SVGs are settled — original-only, shipped in the world, nothing playable in v1; ec-dapp's playable converter migrates into the SDK at P10 (#15 closed → SPECS §Worlds & Views, Token SVGs).
...
**Settled → SPECS §`crawler-api`** (#20 closed, illustrative code included): complete refactor into a **pure contract interface** — parsed results out, callers convert; fully-typed per-world viem contract instances (const-asserted ABIs); `getCardsContract()` + generic ERC-20/ERC-721 helpers; RPC-fallback `console.warn`; `BigIntish` addresses at every boundary; the view-definition machinery deleted; the serializer stays as the api's one non-contract member (#11); the live path is api surface too — poll-based watcher + per-schema payload assemblers, caller-supplied viem clients accepted api-wide (#16 closed); the Starknet seam noted, not designed.

The P3 work **landed** (outcome in `SDK_REFACTOR.md`); the factory names are inventoried in SPECS §Public surface (#7).

---

## `apps/sdk-explorer` — browse tool & API provider — DECIDED direction

**Settled → SPECS §`apps/sdk-explorer`:** the browse-tool role, the dogfooding rule (public SDK surface only), the API-provider shape (#18, closed — same-origin data routes + converted on-chain routes, CORS opt-in via env), the concrete explorer surface (full browse tool: world → SVG-grid index → chamber detail with door navigation; slug-addressed pages with **separator-less route slugs**; data routes keyed by coord/tokenId; the on-chain route as live payload assembler; the raw `/api/read` route deleted), and visual browsing via the original token SVGs (playable form arrives with the P10 converter migration — #15 closed).
...
| **The three chamber-data tiers** | _(settled → SPECS §Data pipeline)_ | No read-through source interface — every tier materializes via `world.import` + pure merge (#16). The provisional `ChamberSource` name retires with it. |
| **Chamber lookup by any key form** | **`ChamberLocator`** _(settled → SPECS §The `Crawler` client)_ | `{ tokenId?, coord?, slug?, compass? }`, first-present-wins; resolved by core's `resolveCoord`. |
| **The sole-world default** | **default world** _(settled → SPECS §The `Crawler` client)_ | `crawler.world()` / hook `worldName?` omitted → the sole registered world; deterministic, never a mutable "current". |
| **Live fetch + payload shaping** | **Payload assembler** (`assembleEcTokenPayload`, …) _(settled → SPECS §`crawler-api`)_ | Per schema, in `crawler-api` — the SDK's single fetch/assembly implementation (cache + live). |
...
| **API reference website** | **`apps/docs`** (vocs) _(new; mechanism decided #12 → SPECS)_ | TSDoc + Twoslash over the exported surface. |
...
| **crawler-react scaffolding** — inert reducer (`SET_SOMETHING`, `chambers: []`), `dispatchChamberData` writing straight into the core global store, `useEvent` DOM bridge, `useConsole*`/`useEffectOnce` misc hooks (`context/CrawlerContext.tsx`, `hooks/`) | _Executed at P2 (keep-lights-on):_ scaffolding deleted — the provider holds the `Crawler`, hooks read it over the coarse subscription; the full hook/component surface lands at P7 (→ SPECS §`crawler-react`). |

Not listed: everything `luw`-only (its unimplemented `slugToCompass`, `Domain` enum, `makeRealmEntryChamberIdFromCoord`) — covered by "`luw` is deleted"; and the chamber query methods (`getMultiple`, static/dynamic counts, `getTokensCoords`) — already in Goals ("enumerate chambers; counts"), enumerated exactly in SPECS §Public surface (#7).
...
- **Documented public surface** — settled → SPECS §Type-system rules (complete TSDoc on every export; Twoslash-verified examples); the generation mechanism is decided (#12 closed → SPECS). Note: today's comments use non-idiomatic `@type` JSDoc tags — convert to proper TSDoc during the typing work.
...
**→ SPECS §The `Crawler` client** — the former strawman is now the settled shape (handle + container, sync static reads, door-based navigation, schema-bound coordinate math, immutable worlds + pure merge + coarse signal) and lives there, illustrative code included. The **exact method inventory** is frozen in SPECS §Public surface (#7).

---

## Data model notes

- **Views:** settled → SPECS §Worlds & Views (#13 closed — absent-view semantics and no-per-view-subpaths included); landed at P2 (`worldInfo`/`tokenCoord`/`chamberData`), `tokenSvg` landed with the P6 builder.
...
- **P2 — Core / client. ✅ LANDED.** Global store + `Options` bag + module/namespace ceremony + DOM events deleted (`./internal` subpath gone); functional read surface (`loadWorld`, pure per-view reads, pure merge); the NEWS library (`coords/news`, ec fixtures pass unchanged) + name→library registry; `createCrawler` + `Crawler` + `WorldHandle` + `Chamber` + coarse subscription + `Converter`/chamber-source interfaces; the one-off migration script (`crawler-data/scripts/migrateWorlds.ts`) rewrote mainnet+goerli to the World shape (277/70 tokens, door `destCoord`/`destTile` derived, validated by `loadWorld` + invariant tests). Consumers ride keep-lights-on passes: api's view machinery deleted early (P3 deletes it anyway) with `formatViewData` brought to spec (replacer, no monkeypatch); react holds a `Crawler` (`useCrawler`/`useWorld`/`useChamber`/`useWorldNames`); explorer re-pointed (its `/api/view` route + converted-read menus parked until P3/P8).
- **P3 — api contract layer. ✅ LANDED.** The typed contract surface (→ SPECS §`crawler-api`; outcome in `SDK_REFACTOR.md`): ABI codegen (artifact JSON → git-ignored const-asserted TS, regenerated before every build/typecheck/test), `getWorldContract` + cards/ERC-20/ERC-721/typed factories, parsed-result helpers (`readTokenMetadata`/`readTotalSupply`/`readOwnerOf`), warned public-RPC fallback, `BigIntish` boundaries; the global RPC registry + untyped read path deleted (`formatViewData` stayed put — #11); explorer `/api/read` re-pointed keep-lights-on. (Watcher/assemblers landed at P7; ownership comes at P10.)
- **P4 — EC cache. ✅ LANDED.** The single private `cache` package (outcome in `SDK_REFACTOR.md`): one generic `fetch:tokens` script archives every token's `tokenURI` via P3 — block-pinned, missing-only, RPC-required, ≤ 10 req/s — into a `json`+`svg` pair per token (SVG prettier-formatted) + a per-`dataDir` `_cache.json` (binding echo + watermark + per-token stamps); mainnet landed at **326 tokens** (goerli unfetchable/frozen). The P3 read helpers gained optional block-pinning (`ReadOptions`). Invalidation landed as an empty schema-level core policy, filled at P7 (the Minted-neighbour staleness pass — #16 closed). _Gates: none — #21/#22 closed (→ SPECS §Data pipeline)._
- **P5 — EC converter. ✅ LANDED.** `EcTokenPayload` → `ChamberData<ec>` (outcome in `SDK_REFACTOR.md`): pure, sync, struct-fed — **no SVG parsing** (the cache embeds the on-chain `chamber` struct, so map facts incl. `seed` ride in the payload; the SVG passes through display-only); readable facts normalized from the traits; the migration's derivations reproduced and pinned by the equivalence suite over the full cache.
- **P6 — EC world data (builder). ✅ LANDED.** The builder (`crawler-data`'s `build:worlds`, outcome in `SDK_REFACTOR.md`) re-emits mainnet from the cache via the canonical serializer (#11) — fully offline, deterministic, `worldInfo` re-stamped, first `TokenSvg` view; goerli stays frozen as migrated (still shipped); `crawler-data` moved to per-world subpath exports (root ships no world JSON); `migrateWorlds.ts` retired. _Gates: none — #6/#22 closed._
- **P7 — react (hook surface + live path). ✅ LANDED.** The full react surface + the cross-package live path (outcome in `SDK_REFACTOR.md`): base-hook + alias model (`useWorldSchema`/`useChamberSchema` → `useWorldEC`/`useChamberEC` + union forms; per-schema type aliases in core; world name optional everywhere — arg → `defaultWorld` → sole world) plus enumeration/info/selector/neighbors/`useTokenSvg` + `<ChamberSvg>`; `watchMints` + schema-dispatched payload assemblers in `crawler-api` (consumed by the cache — one fetch/assembly implementation), caller-supplied viem clients api-wide, the zero-config `liveUpdate` prop (self-contained `useLiveWorld`, `crawler-api` optional peer via dynamic import) + localStorage persistence; the invalidation primitive in core (`getInvalidatedCoords`) + the cache staleness pass, verified against live mainnet. Alias spellings settled (`EC`/`CC` — SPECS §Public surface)._
- **P8 — sdk-explorer. ✅ LANDED.** Rebuilt on the new API (outcome in `SDK_REFACTOR.md`): the **full browse tool** (world index → SVG-grid chamber index → chamber detail with door navigation, separator-less route slugs, goerli data-only behind `hasView`) on the P7 react surface (hooks + `<ChamberSvg>` — the bindings' **reference implementation**) + the **same-origin data API** (the `/api/data` family, the converted `/api/onchain` route, `EXPLORER_CORS_ORIGINS` opt-in) — → SPECS §`apps/sdk-explorer`; the raw `/api/read` scaffold deleted; verified live (cached-vs-live token compare exact). Tailwind + TanStack Query stayed app-side. _Gates: none._
- **P9 — Publish.** Surface **frozen** (#3/#7 → SPECS §Public surface); TSDoc complete + **`apps/docs` stands** (#12: hybrid, gated by `check:docs`); README/CLAUDE.md/PUBLISH.md updated (the publish flow is documented in `PUBLISH.md` §5). _What remains is the **first npm publish** itself — run by the user per the runbook._
...
_None open — #3, #7, and #12 are closed (below); P9 is execution only._
...
- **#7 — CLOSED.** The public surface is **frozen → SPECS §Public surface**: the full per-package export inventory + the `Crawler`/`WorldHandle`/`Chamber` method inventory, grounded in the shipped barrels; the parked residuals resolved there (#19's `isDynamic` query surface, #20's factory names, the `EC`/`CC` alias spellings, the react hook names). Ratification outcomes: the raw react context (`CrawlerContext`/`CrawlerContextType`) is **internal**; `bigintish` ships as the **`bi` namespace** — prefix-dropped members (`bi.toAddress`, `bi.equals`, …; `toDecimalString` not `toString` — prototype shadowing; `fromBinaryArray` for the packing direction), types + `InvalidBigIntishError` top-level, **no subpath export** in v1; prettier stays a runtime dep of the api (the `#11` footprint revisit — standalone entries, tree-shaken when unused).
- **#3 — CLOSED.** Boundary criterion: everything that deals directly with `ChamberData` lives in `crawler-core` — a complete game needs only **core + one world from `crawler-data`**; `crawler-api`/`crawler-react` strictly optional. The package split stays `core`/`data`/`api`/`react` (published names, ec-dapp's consumption); the workspace grows around them (private `cache`, converters + builder in `data`, watcher + owner helpers in `api`) — the published set unchanged. The per-package export inventory is #7's freeze. → SPECS §Package map, §Public surface.
- **#12 — CLOSED.** Reference pages are **hybrid (c)**: a **generated symbol reference** (TypeDoc-style step emitting MDX from the TSDoc'd surface, rendered by vocs) + **hand-authored MDX guides/overviews** carrying the narrative; examples Twoslash-verified. The `apps/docs` build **gates CI** — a broken example or an undocumented export turns CI red (what makes the documented-surface rule enforceable). Rejected: fully hand-authored (manual upkeep as the surface grows) and fully generated (coarse output, no narrative). → SPECS §Package map (`apps/docs`), §Type-system rules.
- **#16 — CLOSED.** Live-path mechanics: a **poll-based mint watcher** over `totalSupply` + **per-schema payload assemblers** in `crawler-api` — the SDK's single fetch/assembly implementation, consumed by the cache fetch too (its inline assembly migrates into the api); the api accepts **caller-supplied viem clients** beside `rpcUrl`; `crawler-react` persists **converted** records keyed by world binding (private versioned bigint-safe format), pruned on load as static worlds catch up, and wires the live path **zero-config** — a `liveUpdate` provider prop calling the self-contained `useLiveWorld` (the provider stays thin), which reaches `crawler-api` as an **optional peer via dynamic import** (games without live never install it); the **Minted-neighbour invalidation** is a schema-declared policy with one pure core primitive (`getInvalidatedCoords`) serving the live client and the cache staleness pass; the provisional async `ChamberSource` read-through interface is **dropped** — every tier materializes via `world.import` + pure merge, reads stay sync. Rejected: event-log subscriptions (polling covers mints on any RPC), on-demand fetch-on-miss as core surface, TanStack Query in the SDK (app-side only), ox/viem-core switch (viem 2 + client acceptance is the compatibility answer), developer-injected watcher/assembler in the live hook (the SDK already knows everything — zero-config won). → SPECS §`crawler-react`, §Data pipeline, §Schemas, §`crawler-api`.
...
- **#9 — CLOSED.** Wrapper shape **(a): per-world handle + `Crawler` container**, method-style over the functional core; sync for all static data; worlds by name; chamber back-pointer; no mutable "current world". Rejected (b) one-client-explicit-refs (verbose call sites) and (c) pure-functions-only (the world registry and `world.import` have no home; `import` is a reserved word as a bare function). Exact method inventory → SPECS §Public surface (#7). → SPECS §The `Crawler` client.
