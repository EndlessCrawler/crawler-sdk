...
| **Per-world on-chain snapshot** | **the `cache` package** _(settled → SPECS §Data pipeline)_ | One private, contract-agnostic package; one dir per world under `cache/data/<dataDir>/<network>`. Pure `tokenURI` archive — layout & fetch discipline settled (#21). |
...
- **P4 — EC cache.** The single `cache` package: fetch every token's `tokenURI` on-chain via P3, store the `json`+`svg` pair per token + a per-network `_cache.json` (mainnet only — goerli unfetchable/frozen), missing-only incremental fetch (invalidation deferred to #16). Define the EC **world schema + views** for real — the first `DataSchema` instance exercised. _Gates: none — #21/#22 closed (→ SPECS §Data pipeline)._
