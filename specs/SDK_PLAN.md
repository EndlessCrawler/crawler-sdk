...
**Status:** _Implementation underway — **P1–P6 landed**_ (core rewritten to the settled spec, worlds migrated, the api contract layer, the EC on-chain cache, the EC converter, and the builder + per-world exports; execution detail in `specs/SDK_REFACTOR.md`). This refactor runs **after** the V2 modernization (`specs/V2_PLAN.md`) landed its stack work, but **before anything is published to npm** — the first publish happens only once this refactor is done (P9). V2 made the stack modern and correct; this plan makes the *API* the one we actually want, and that shipped API is what goes to npm.
...
- **P7 — react (hook surface + live path).** Runs **before** the explorer (swapped so the explorer lands once, on the final react surface). The provider stays (+ `defaultWorld`); the hook surface grows on the **base-hook + alias model** (`useWorldSchema`/`useChamberSchema` bases → `useWorldEC`-style aliases + union forms, matching per-schema **type aliases** in core — `WorldHandleEC`, `ChamberEC`, …; subject-first args, world name optional everywhere — single-world default; alias spellings final at #7) plus enumeration/info/selector/neighbors/`useTokenSvg` + `<ChamberSvg>`; the live path lands across packages: poll-based `watchMints` + per-schema payload assemblers in `crawler-api` (also consumed by the cache — one fetch/assembly implementation), caller-supplied viem clients accepted api-wide, the zero-config `liveUpdate` provider prop (self-contained `useLiveWorld`, `crawler-api` as optional peer via dynamic import) + localStorage persistence in `crawler-react`, the invalidation primitive in core + the cache staleness pass (#16 closed → SPECS §`crawler-react`, §Data pipeline, §Schemas, §`crawler-api`; map in `SDK_REFACTOR.md`). _Gates: none — #16 closed._
- **P8 — sdk-explorer.** Rebuild on the new API: the **full browse tool** (world → SVG-grid chamber index → chamber detail with door navigation) + the **same-origin data API** — data routes and converted on-chain routes (#18 closed and concretized → SPECS §`apps/sdk-explorer`; map in `SDK_REFACTOR.md`). Consumes the P7 react surface (hooks + `<ChamberSvg>`) as its **reference implementation**; Tailwind + TanStack Query stay app-side. _Gates: none._
