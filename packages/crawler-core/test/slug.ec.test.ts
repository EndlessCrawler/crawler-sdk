...
describe('slug.ec', () => {
  ...
  const _validateSlug = (pair: TestPair, separator: SlugSeparator) => {
    ...
    expect(
      asCompass,
      `Slug [${slug}] compass [${JSON.stringify(asCompass)}] should not be null`,
    ).not.toBe(null);
    expect(
      client.compassEquals(asCompass, pair.compass),
      `Slug [${slug}] compass [${JSON.stringify(asCompass)}] is not [${JSON.stringify(pair.compass)}]`,
    ).toBe(true);
    // convert to coord
    ...
    if (!pair.forwardOnly) {
      ...
      expect(
        slugFromCompass,
        `Compass [${JSON.stringify(pair.compass)}] slug is not [${slug}]`,
      ).toBe(slug.toUpperCase());
  ...
  const _invalidateSlug = (slug: string) => {
    ...
    // convert to Compass
    const asCompass = client.slugToCompass(slug);
    expect(asCompass, `Slug [${slug}] compass [${JSON.stringify(asCompass)}] is not null`).toBe(
      null,
    );
    // convert to coord
    const asCoord = client.slugToCoord(slug);
