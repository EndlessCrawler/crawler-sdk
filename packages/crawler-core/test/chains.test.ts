...
describe('* chains', () => {
  ...
  it('lookups', () => {
    expect(
      chainIds.length,
      'ChainIdToNetworkName and NetworkNameToChainId must be the same size',
    ).toBe(allChainIds.length);
    expect(
      networkNames.length,
      'ChainIdToNetworkName and NetworkNameToChainId must be the same size',
    ).toBe(allNetworkNames.length);

    expect(
      chainIds.length,
      'ChainIdToNetworkName and NetworkNameToChainId must be the same size',
    ).toBe(networkNames.length);

    const uniqueChainIds = [...new Set(chainIds)];
    expect(uniqueChainIds.length, 'ChainIdToNetworkName has duplicated items').toBe(
      chainIds.length,
    );

    const uniqueNetworkNames = [...new Set(networkNames)];
    expect(uniqueNetworkNames.length, 'NetworkNameToChainId has duplicated items').toBe(
      networkNames.length,
    );
    ...
    for (let i = 0; i < chainIds.length; ++i) {
      ...
      expect(chainId, 'ChainIdToNetworkName / NetworkNameToChainId inconsistency').toBe(
        NetworkNameToChainId[networkName],
      );
    }
  });
});
