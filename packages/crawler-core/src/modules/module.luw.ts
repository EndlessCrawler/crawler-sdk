...
export namespace LootUnderworld {
  ...
  export class Module extends ModuleBase implements ModuleInterface {
    ...
    validateCompass(compass: Compass | null): boolean {
      ...
      if (
        compass.tokenId &&
        (compass.domainId == null ||
          compass.tokenId < 1 ||
          compass.tokenId > DomainTokenCount[compass.domainId])
      )
        return false;
      return true;
    }
    ...
    compassToCoord(compass: Compass | null): bigint {
      ...
      if (compass && this.validateCompass(compass)) {
        if (compass.domainId && compass.domainId > 0)
          result += BigInt(compass.domainId) << CoordOffset.DomainId;
        if (compass.tokenId && compass.tokenId > 0)
          result += BigInt(compass.tokenId) << CoordOffset.TokenId;
        if (compass.over && compass.over > 0) result += BigInt(compass.over) << CoordOffset.Over;
        if (compass.under && compass.under > 0)
          result += BigInt(compass.under) << CoordOffset.Under;
        if (compass.north && compass.north > 0)
          result += BigInt(compass.north) << CoordOffset.North;
    ...
    compassToSlug(
      ...
      separator: SlugSeparator = _defaultSlugSeparator,
    ): string | null {
      let result = '';
      if (compass && this.validateCompass(compass)) {
        if (compass.tokenId) {
