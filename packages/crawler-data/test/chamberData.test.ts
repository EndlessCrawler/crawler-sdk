...
describe('chamberData', () => {
  ...
  it('getView() getData()', () => {
    ...
    expect(() => client.chamberData.getView({ dataSetName: 'xxx' as NetworkName })).toThrow(
      'InvalidDataSetError',
    );
  });
  ...
  it('Options.chain', () => {
    ...
    const coords_g = client.tokenIdToCoord.get(count_g, {
      dataSetName: NetworkName.Goerli,
    }) as ChamberCoords;
    expect(BigInt(coords_g.coord)).toBeGreaterThan(0n);
    const ch_g = client.chamberData.get(coords_g.coord, {
      dataSetName: NetworkName.Goerli,
    }) as ChamberData;
