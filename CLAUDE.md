...
- **`specs/SDK_PLAN.md`** — the **SDK refactor** plan of record: reshapes the API (functional core + thin wrapper, schemas, lazy datasets, documented surface) *after* V2 lands but *before* the first npm publish. Still in the idea-capture/brainstorm stage — driven by the **`/sdk-plan`** slash command (`.claude/commands/sdk-plan.md`), which collects and organizes the user's ideas into it. All refactor state lives in that file. Do not start refactor implementation until the user says so.
