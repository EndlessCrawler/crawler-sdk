...
- **crawler-api** — on-chain web3 layer, **viem 2 only**, plus contract ABIs in `contracts/` and `artifacts/`; also home of **`formatViewData`**, the canonical dataset serializer (must stay in this package — SPECS §Canonical serialization). The read path is interim (`readContractOrThrow` over cached `PublicClient`s, caller-supplied RPC urls via `setRpcUrl`/`setRpcUrls`, Mainnet default) — the **P3 refactor** replaces it with per-world typed viem contract instances.
