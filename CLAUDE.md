...
- **crawler-react** — `CrawlerProvider` (holds an explicit `Crawler`) + hooks (`useCrawler`, `useWorld`, `useChamber`, `useWorldNames`). Keep-lights-on P2 shape; the full hook/component surface + live path (base hooks + schema aliases `useWorldEC`/`useChamberEC`, world name optional everywhere, the zero-config `liveUpdate` provider prop over a self-contained `useLiveWorld` — `crawler-api` as optional peer via dynamic import — `<ChamberSvg>`, localStorage persistence) lands at the SDK-refactor P7 (→ SPECS §`crawler-react`).
