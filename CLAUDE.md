...
- **crawler-data** — static worlds in the settled World shape (`src/worlds/mainnet.json`, `goerli.json`), exported as `WorldJson` values (`mainnetWorld`, `goerliWorld`, `allWorlds`), plus the **`ec` converter** (SDK-refactor P5, `src/converters/ec/`): `ecConverter` + payload types (`EcTokenPayload` = `tokenId` + `metadata: EcTokenMetadata & EcChamberMetadata` + `svg`) — pure, sync, core-only; map facts from the embedded `metadata.chamber` struct, readable facts from the traits, **no SVG parsing**; bad payloads throw `TokenConversionError`. All interim root exports; per-world subpaths land at P6. The equivalence suite (`test/converter.ec.test.ts`) pins the converter against the migrated world — exact on static chambers, monotone lock evolution on the 41 dynamic chambers the chain moved since the legacy snapshot. `scripts/migrateWorlds.ts` is the one-off P2 migration that produced the worlds (retired at P6). Goerli is **frozen as migrated** (dead chain).
